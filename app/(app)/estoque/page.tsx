...
export default function EstoquePage() {
  ...
  useEffect(()=>{if(!profile?.church_id)return;load();createClient().from('locations').select('id,name').eq('church_id',profile.church_id).eq('is_active',true).order('name').then(({data})=>{if(data)setLocations(data)});createClient().from('stock_movements').select('product_id,location_id,type,quantity').eq('church_id',profile.church_id).not('location_id','is',null).then(({data})=>{if(data){const m={};const bal={};data.forEach((r)=>{const key=r.product_id+'|'+r.location_id;if(!bal[key])bal[key]=0;if(r.type==='in')bal[key]+=r.quantity;if(r.type==='out')bal[key]-=r.quantity;if(!m[r.location_id])m[r.location_id]=new Set();m[r.location_id].add(r.product_id)});setProductsByLoc(m);setLocBalance(bal)}})},[profile?.church_id])
