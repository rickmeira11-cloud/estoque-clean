...
export default function IgrejasPage() {
  const [churches,setChurches]=useState<(Church&{_users:number})[]>([])
  ...
  async function load(){
    ...
    if(data){const wc=await Promise.all(data.map(async c=>{const {count}=await sb.from('profiles').select('*',{count:'exact',head:true}).eq('church_id',c.id);return{...c,_users:count||0}}));setChurches(wc as any)}
    setLoading(false)
  }
  function openNew(){setEditId(null);setForm(blank);setError(null);setShowForm(true)}
  function openEdit(c:Church){setEditId(c.id);setForm({name:c.name,slug:c.slug||'',city:c.city||'',state:c.state||''});setError(null);setShowForm(true)}
  async function save(){
  ...
  async function toggleActive(c:Church){await createClient().from('churches').update({is_active:!c.is_active}).eq('id',c.id);await load()}
  const L={display:'block' as const,fontSize:'11px',color:'var(--text-3)',marginBottom:'5px'}
  return (
    <div style={{maxWidth:'800px'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'24px'}}>
        <div><h1 style={{fontSize:'22px',fontWeight:'600'}}>Igrejas</h1><p style={{fontSize:'13px',color:'var(--text-3)',marginTop:'4px'}}>Organizações cadastradas no sistema</p></div>
      ...
      {showForm&&(<div className="fade-up" style={{background:'var(--bg-card)',border:'1px solid var(--border-md)',borderRadius:'12px',padding:'24px',marginBottom:'20px'}}>
          ...
          {churches.length===0?<div style={{textAlign:'center',padding:'40px',fontSize:'13px',color:'var(--text-3)'}}>Nenhuma igreja cadastrada.</div>:churches.map(c=>(
            <div key={c.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'16px 18px',borderRadius:'12px',background:'var(--bg-card)',border:`1px solid ${c.is_active?'var(--border)':'rgba(255,255,255,0.03)'}`,opacity:c.is_active?1:0.5}}>
              <div style={{display:'flex',alignItems:'center',gap:'14px'}}>
                ...
                <div><div style={{fontSize:'14px',fontWeight:'500'}}>{c.name}</div><div style={{fontSize:'11px',color:'var(--text-3)',marginTop:'3px',display:'flex',gap:'10px'}}><span>{[c.city,c.state].filter(Boolean).join(', ')||'Localização não informada'}</span><span style={{fontFamily:'monospace'}}>{c.slug}</span><span style={{color:'var(--text-2)'}}>{c._users} usuário(s)</span></div></div>
              </div>
