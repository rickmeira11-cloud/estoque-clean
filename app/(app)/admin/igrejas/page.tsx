...
export default function IgrejasPage() {
  ...
  async function save() {
    ...
    const { error: err } = editId
      ? await createClient().from('churches').update(payload).eq('id', editId)
      : await createClient().from('churches').insert(payload)
    if (err) setError(err.code === '23505' ? 'Já existe uma igreja com esse slug.' : err.message)
          ...
          {churches.map(c => (
            <div key={c.id} style={{ borderRadius: '12px', background: 'var(--bg-card)', border: `1px solid ${c.is_active ? 'var(--border)' : 'rgba(255,255,255,0.03)'}`, opacity: c.is_active ? 1 : 0.5, overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'var(--brand-dim)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: '700', color: 'var(--brand-light)', flexShrink: 0 }}>
