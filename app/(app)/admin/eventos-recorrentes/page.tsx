...
type Rule = { id: string; name: string; weekday: number; description: string | null; is_active: boolean }
...
export default function EventosRecorrentesPage() {
  ...
  async function handleSave() {
    ...
    await createClient().from('recurring_events').insert({
      church_id: profile!.church_id, name: name.trim(), weekday: parseInt(weekday), description: desc || null,
    })
    setShowForm(false); setName(''); setWeekday('0'); setDesc('')
    ...
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'20px', flexWrap:'wrap', gap:'12px' }}>
        <div>
          ...
          <p style={{ fontSize:'13px', color:'var(--text-3)', marginTop:'4px' }}>Crie regras para gerar eventos automaticamente (2 dias antes)</p>
        </div>
        {isAdmin && <button onClick={() => setShowForm(true)} style={{ padding:'8px 18px', borderRadius:'var(--radius-sm)', background:'var(--brand)', color:'#fff', border:'none', cursor:'pointer', fontSize:'13px', fontWeight:'500' }}>+ Nova regra</button>}
      </div>
      ...
      <div style={{ marginBottom:'20px', padding:'12px 16px', borderRadius:'var(--radius)', background:'var(--info-dim)', border:'1px solid rgba(96,165,250,0.2)', fontSize:'13px', color:'var(--text-2)' }}>
        💡 As regras ativas geram eventos automaticamente toda vez que faltam 2 dias para o dia da semana configurado. Ex: regra "Culto de Domingo" cria o evento toda sexta-feira.
      </div>
      ...
      {showForm && (
        <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:'var(--radius)', padding:'20px', marginBottom:'20px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:'12px', marginBottom:'12px' }}>
            <div><label style={L}>Nome do evento *</label><input value={name} onChange={e => setName(e.target.value)} placeholder="Ex: Culto de Domingo"/></div>
            <div>
              <label style={L}>Dia da semana do evento *</label>
            ...
            <div key={r.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'14px 18px', borderRadius:'var(--radius)', background:'var(--bg-card)', border:`1px solid ${r.is_active ? 'var(--border)' : 'rgba(255,255,255,0.04)'}`, opacity:r.is_active?1:0.55, gap:'12px', flexWrap:'wrap' }}>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                  ...
                  <span style={{ fontSize:'11px', padding:'2px 10px', borderRadius:'99px', background:'var(--brand-dim)', color:'var(--brand-light)', fontWeight:'600' }}>{WEEKDAYS[r.weekday]}</span>
                  {!r.is_active && <span style={{ fontSize:'10px', padding:'2px 8px', borderRadius:'99px', background:'var(--bg-3)', color:'var(--text-3)' }}>Pausada</span>}
                </div>
                {r.description && <div style={{ fontSize:'12px', color:'var(--text-3)', marginTop:'3px' }}>{r.description}</div>}
              </div>
              {isAdmin && (
