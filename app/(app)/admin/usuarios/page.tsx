...
export default function UsuariosPage() {
  ...
  async function save() {
    ...
    if (editId) {
      ...
      if (!form.email.trim()) { setError('E-mail obrigatório'); setSaving(false); return }
      const { data: { session } } = await sb.auth.getSession()
      const token = session?.access_token || ''
      const primaryChurch = userChurches[0]
      const res = await fetch('/api/usuarios', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
        body: JSON.stringify({ ...form, church_id: primaryChurch?.church_id || profile?.church_id })
      })
...
export default function UsuariosPage() {
  ...
  function addChurch() {
    const remaining = churches.filter(c => !userChurches.find(uc => uc.church_id === c.id))
    if (remaining.length === 0) return
    setUserChurches(prev => [...prev, { church_id: remaining[0].id, role: 'operator', is_active: true, church: remaining[0] }])
  }

  function removeChurch(idx) {
    if (userChurches.length === 1) { setError('É necessário pelo menos uma igreja'); return }
    setUserChurches(prev => prev.filter((_, i) => i !== idx))
  }

  function updateChurch(idx, field, value) {
    setUserChurches(prev => prev.map((uc, i) => {
      if (i !== idx) return uc
      if (field === 'church_id') { const church = churches.find(c => c.id === value); return { ...uc, church_id: value, church } }
  ...
  if (!isAdmin) return (<div><h1 style={{ fontSize: '22px', fontWeight: '600' }}>Usuários</h1><p style={{ fontSize: '13px', color: 'var(--text-3)', marginTop: '8px' }}>Sem permissão.</p></div>)

  return (
    <div style={{ maxWidth: '800px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', gap: '12px' }}>
        <div><h1 style={{ fontSize: '22px', fontWeight: '600' }}>Usuários</h1><p style={{ fontSize: '13px', color: 'var(--text-3)', marginTop: '4px' }}>Gerencie acessos e vínculos com igrejas</p></div>
        <button onClick={openNew} style={{ padding: '9px 18px', background: 'var(--brand)', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '500', cursor: 'pointer', flexShrink: 0 }}>+ Novo usuário</button>
      </div>
      ...
      {senhaGerada && (
        <div className="fade-up" style={{ marginBottom: '16px', padding: '16px 20px', borderRadius: '12px', background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.3)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--brand-light)', marginBottom: '4px' }}>Usuário criado!</div>
              <div style={{ fontSize: '12px', color: 'var(--text-2)' }}>Senha temporária:</div>
      ...
      {showForm && (
        <div className="fade-up" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-md)', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div><label style={L}>Papel padrão</label><select value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))}>{Object.entries(ROLES).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></div>
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                <label style={{ fontSize: '12px', fontWeight: '500', color: 'var(--text-2)' }}>Igrejas vinculadas</label>
                <button type="button" onClick={addChurch} disabled={churches.length === userChurches.length} style={{ padding: '3px 10px', background: 'var(--brand-dim)', border: '1px solid var(--brand)', borderRadius: '6px', fontSize: '11px', color: 'var(--brand-light)', cursor: 'pointer', opacity: churches.length === userChurches.length ? 0.5 : 1 }}>+ Adicionar igreja</button>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {userChurches.map((uc, idx) => (
                  <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 150px auto', gap: '8px', alignItems: 'center', padding: '10px 12px', borderRadius: '8px', background: 'var(--bg-3)', border: '1px solid var(--border)' }}>
                    ...
                    <button type="button" onClick={() => removeChurch(idx)} style={{ padding: '6px 10px', background: 'transparent', border: '1px solid var(--border)', borderRadius: '6px', fontSize: '12px', color: 'var(--empty)', cursor: 'pointer' }}>✕</button>
                  </div>
                ))}
              </div>
            </div>
            {!editId && <div style={{ padding: '10px 12px', borderRadius: '8px', background: 'var(--info-dim)', fontSize: '12px', color: 'var(--info)', border: '1px solid rgba(99,102,241,0.2)' }}>Uma senha temporária será gerada após a criação.</div>}
          </div>
          <div style={{ display: 'flex', gap: '10px', marginTop: '18px' }}>
            <button onClick={save} disabled={saving} style={{ padding: '9px 20px', background: 'var(--brand)', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '500', cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>{saving ? 'Salvando...' : editId ? 'Atualizar' : 'Criar usuário'}</button>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '40px', fontSize: '13px', color: 'var(--text-3)', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px' }}>Nenhum usuário.</div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {users.map(u => (
