...
export default function RelatoriosPage() {
  ...
  async function loadMinisterios() {
    ...
    since.setDate(since.getDate() - parseInt(minPeriod))
    const { data } = await sb
      .from('stock_movements')
      .select('quantity,ministry:ministries(id,name),product:products(name,category)')
      .eq('church_id', profile!.church_id)
      .eq('type', 'out')
    ...
    if (data) {
      ...
      data.forEach((m: any) => {
        const mn = m.ministry?.name || 'Sem ministério'
        const mn_id = m.ministry?.id || 'none'
        if (!map[mn_id]) map[mn_id] = { name: mn, total: 0, produtos: {} }
        map[mn_id].total += m.quantity
        const prod = m.product?.name || 'Desconhecido'
        map[mn_id].produtos[prod] = (map[mn_id].produtos[prod] || 0) + m.quantity
      })
      setMinRows(Object.values(map).sort((a,b) => b.total - a.total))
    }
    setMinLoading(false)
  }
          ...
          {tab === 'ministerios' && (
            <div>
              <div style={{ display:'flex', gap:'8px', marginBottom:'16px', flexWrap:'wrap', alignItems:'center' }}>
                ...
                {[{v:'7',l:'7 dias'},{v:'30',l:'30 dias'},{v:'90',l:'3 meses'}].map(o => (
                  <button key={o.v} onClick={() => setMinPeriod(o.v)}
                    style={{ padding:'5px 14px', borderRadius:'99px', border:'1px solid', fontSize:'12px', cursor:'pointer',
                      background: minPeriod===o.v ? 'var(--brand)' : 'transparent',
                      color: minPeriod===o.v ? '#fff' : 'var(--text-3)',
                      borderColor: minPeriod===o.v ? 'var(--brand)' : 'var(--border)' }}>
                    {o.l}
                  </button>
                ))}
              </div>
              {minLoading ? (
                ...
                <div style={{ textAlign:'center', padding:'40px', color:'var(--text-3)' }}>
                  ...
                  <div>Nenhuma saída vinculada a ministérios neste período.</div>
                  <div style={{ fontSize:'12px', color:'var(--text-3)', marginTop:'6px' }}>Vincule saídas a ministérios na tela de Movimentação.</div>
                </div>
              ) : (
                      ...
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'12px' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                          <div style={{ width:'36px', height:'36px', borderRadius:'8px', background:'var(--brand-dim)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--brand-light)" strokeWidth="2"><path d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                          </div>
                          <div>
                            <div style={{ fontSize:'14px', fontWeight:'600', color:'var(--text-1)' }}>{m.name}</div>
                            <div style={{ fontSize:'11px', color:'var(--text-3)' }}>{Object.keys(m.produtos).length} produto(s) consumido(s)</div>
                          </div>
                        </div>
                        <div style={{ textAlign:'right' }}>
                          <div style={{ fontSize:'22px', fontWeight:'700', color:'var(--empty)', fontFamily:'var(--font-mono)' }}>{m.total}</div>
