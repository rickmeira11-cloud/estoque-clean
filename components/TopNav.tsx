...
const NAV = [
  ...
  { href:'/movimentacoes', label:'Movimentar',   icon:'M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4' },
  ...
  { href:'/mural',         label:'Ministérios',  icon:'M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75M9 7a4 4 0 110 8 4 4 0 010-8z' },
]

const NAV_CADASTROS = [
  { href:'/admin/igrejas',   label:'Igrejas',      icon:'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-2 10v-5a1 1 0 00-1-1h-2a1 1 0 00-1 1v5m4 0H9' },
  { href:'/ministerios',     label:'Ministérios',  icon:'M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75M9 7a4 4 0 110 8 4 4 0 010-8z' },
...
export function TopNav() {
  ...
  useEffect(() => { setMenuOpen(false); setAlertOpen(false); setCadastrosOpen(false) }, [pathname])
  ...
  const isCadastrosActive = NAV_CADASTROS.some(n => active(n.href))
        ...
        <div style={{display:'flex',alignItems:'center',gap:'2px',flex:1}} className="desktop-nav">
          ...
          {isAdmin && (
            <div ref={cadastrosRef} style={{position:'relative',flexShrink:0}}>
              <button onClick={() => setCadastrosOpen(o => !o)} style={{
                display:'flex', alignItems:'center', gap:'6px',
                padding:'6px 12px', borderRadius:'var(--radius-sm)',
                fontSize:'13px', fontWeight: isCadastrosActive ? '500' : '400',
                color: isCadastrosActive ? 'var(--text-1)' : 'var(--text-2)',
                background: isCadastrosActive || cadastrosOpen ? 'var(--bg-3)' : 'transparent',
                border: isCadastrosActive ? '1px solid var(--border-md)' : '1px solid transparent',
                cursor:'pointer', transition:'all 0.15s', whiteSpace:'nowrap',
              }}>
                <span style={{color: isCadastrosActive ? 'var(--brand-light)' : 'var(--text-3)'}}>
                ...
                {isCadastrosActive && <span style={{width:'4px',height:'4px',borderRadius:'50%',background:'var(--brand-light)',marginLeft:'2px'}}/>}
              </button>
